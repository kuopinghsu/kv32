/* ============================================================================
 * spike/plugin_magic.cc – Spike plugin for the KV32 "Magic" device
 *
 * Base address : KV_MAGIC_BASE  (0xFFFF_0000)
 * Window       : KV_MAGIC_SIZE  (64 KB)
 *
 * Register offsets (from kv_platform.h):
 *   KV_MAGIC_EXIT_OFF     (0xFFF0)  write exit-code encoding
 *   KV_MAGIC_CONSOLE_OFF  (0xFFF4)  write low byte → stdout
 *
 * Enables non-HTIF firmware binaries to produce console output and exit
 * cleanly when run under Spike with the kv32_soc plugin set.
 *
 * Load with:
 *   spike --extlib=build/spike_plugin_magic.so \
 *         --device=kv32_magic,0x$(printf '%X' KV_MAGIC_BASE) ...
 * =========================================================================*/
...
static void plugin_init() {
    register_mmio_plugin("kv32_magic", &plugin);
}
