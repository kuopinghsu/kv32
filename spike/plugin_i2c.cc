...
//   CTRL_STOP sets IS[2] (stop_done) which is sticky; cleared by SW W1C write to IS
...
private:
    ...
    // update_meip() derives the PLIC level from the current is_/ie_ state.
    // IS[2] (STOP_DONE) is sticky – it stays set in is_ until the firmware
    // clears it via a W1C write to IS (reg_write IS_OFF).  There is no
    // auto-consume here; the firmware's kv_i2c_irq_status() call reads IS
    // and then writes the same value back, which clears STOP_DONE via W1C
    // before plic_complete() is called, so the PLIC sees level=0 on exit.
    //
    // RX_READY is level-triggered (fifo non-empty keeps level high); no
    ...
    void update_meip()
    {
        ...
        const bool overall = stop_active || rx_pending;
    ...
    uint32_t reg_read(reg_t addr)
    {
        ...
        case KV_I2C_IS_OFF: {
            // Return current IS; STOP_DONE sticky bit remains set until
            // firmware performs a W1C write to IS (reg_write IS_OFF).
            return is_;
        }
