...
class kv32sim(pluginTemplate):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # -- plugin-specific --------------------------------------------------
        self.dut_exe = os.path.join(self.project_root, 'build', 'kv32sim')
        # ---------------------------------------------------------------------
    ...
    def build(self, isa_yaml, platform_yaml):
        ...
        self.isa_sim = self.isa.replace('_zifencei', '')  # kv32sim does not accept _zifencei
    ...
    def _make_run_cmd(self, elf, sig_file):
        ...
        ).format(exe=self.dut_exe, isa=self.isa_sim, sig=sig_file, elf=elf)

    def _make_trace_cmd(self, elf, sig_file, trace_file):
        # kv32sim writes trace via --rtl-trace --log; stdout+stderr to log
        return (
            '{exe} --isa={isa} --rtl-trace --log={trace}'
            ' +signature={sig} +signature-granularity=4 {elf}'
            ' &> {log}'
        ).format(exe=self.dut_exe, isa=self.isa_sim,
                 trace=trace_file, sig=sig_file, elf=elf,
                 log=sig_file + '.log')
