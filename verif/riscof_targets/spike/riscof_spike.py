...
class spike(pluginTemplate):
    ...
    def _make_trace_cmd(self, elf, sig_file, trace_file):
        # spike writes commit log to stderr; redirect stderr to trace file
        return (
            '{exe} --pc=0x80000000 --log-commits --isa={isa}'
            ' +signature={sig} +signature-granularity=4 {elf}'
            ' > {log} 2>{trace}'
        ).format(exe=self.dut_exe, isa=self.isa_sim,
                 sig=sig_file, elf=elf,
                 log=sig_file + '.log', trace=trace_file)
