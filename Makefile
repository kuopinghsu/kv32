...
# - SRAM model: proportional to the worst memory latency path
#   (max(MEM_READ_LATENCY, MEM_WRITE_LATENCY)).
...
# Compute a latency scale factor once so RTOS tuning tracks whichever side
# (read or write) is slower.
RTOS_LATENCY_SCALE ?= $(shell if [ $(MEM_READ_LATENCY) -gt $(MEM_WRITE_LATENCY) ]; then echo $(MEM_READ_LATENCY); else echo $(MEM_WRITE_LATENCY); fi)
...
ifeq ($(MEM_DUAL_PORT),0)
RTOS_T4_TICK_FAST ?= $(shell expr 30000 \* $(RTOS_LATENCY_SCALE))
else
RTOS_T4_TICK_FAST ?= $(shell expr 5000 \* $(RTOS_LATENCY_SCALE))
endif
endif
...
# Branch predictor defaults and overrides
BP_EN     ?= 1
BTB_SIZE  ?= 32
BHT_SIZE  ?= 64
RAS_EN    ?= 1
...
ifdef BTB_SIZE
	VERILATOR_FLAGS += -pvalue+BTB_SIZE=$(BTB_SIZE)
endif
ifdef BHT_SIZE
	VERILATOR_FLAGS += -pvalue+BHT_SIZE=$(BHT_SIZE)
endif
ifdef RAS_EN
	VERILATOR_FLAGS += -pvalue+RAS_EN=$(RAS_EN)
endif
ifdef RAS_DEPTH
	VERILATOR_FLAGS += -pvalue+RAS_DEPTH=$(RAS_DEPTH)
endif
...
VERILATOR_FLAGS += -CFLAGS "-DICACHE_EN=$(ICACHE_EN) -DICACHE_SIZE=$(ICACHE_SIZE) -DICACHE_LINE_SIZE=$(ICACHE_LINE_SIZE) -DICACHE_WAYS=$(ICACHE_WAYS) -DDCACHE_EN=$(DCACHE_EN) -DDCACHE_SIZE=$(DCACHE_SIZE) -DDCACHE_LINE_SIZE=$(DCACHE_LINE_SIZE) -DDCACHE_WAYS=$(DCACHE_WAYS) -DDCACHE_WRITE_BACK=$(DCACHE_WRITE_BACK) -DBP_EN=$(BP_EN) -DBTB_SIZE=$(BTB_SIZE) -DBHT_SIZE=$(BHT_SIZE) -DRAS_EN=$(RAS_EN) -DRAS_DEPTH=$(RAS_DEPTH)"
RTL_BUILD_PARAMS = FAST_MUL=$(FAST_MUL) FAST_DIV=$(FAST_DIV) BP_EN=$(BP_EN) BTB_SIZE=$(BTB_SIZE) BHT_SIZE=$(BHT_SIZE) RAS_EN=$(RAS_EN) RAS_DEPTH=$(RAS_DEPTH) ICACHE_EN=$(ICACHE_EN) ICACHE_SIZE=$(ICACHE_SIZE) ICACHE_LINE_SIZE=$(ICACHE_LINE_SIZE) ICACHE_WAYS=$(ICACHE_WAYS) DCACHE_EN=$(DCACHE_EN) DCACHE_SIZE=$(DCACHE_SIZE) DCACHE_LINE_SIZE=$(DCACHE_LINE_SIZE) DCACHE_WAYS=$(DCACHE_WAYS) DCACHE_WRITE_BACK=$(DCACHE_WRITE_BACK) DCACHE_WRITE_ALLOC=$(DCACHE_WRITE_ALLOC) ASSERT=$(ASSERT) DEBUG=$(DEBUG) DEBUG_GROUP=$(DEBUG_GROUP) COVERAGE=$(COVERAGE) MEM_READ_LATENCY=$(MEM_READ_LATENCY) MEM_WRITE_LATENCY=$(MEM_WRITE_LATENCY) MEM_DUAL_PORT=$(MEM_DUAL_PORT) MEM_TYPE=$(MEM_TYPE) STRETCH=$(STRETCH)
...
VERILATOR_LINT_FLAGS += -pvalue+BP_EN=$(BP_EN) -pvalue+BTB_SIZE=$(BTB_SIZE)
VERILATOR_LINT_FLAGS += -pvalue+BHT_SIZE=$(BHT_SIZE) -pvalue+RAS_EN=$(RAS_EN)
VERILATOR_LINT_FLAGS += -pvalue+RAS_DEPTH=$(RAS_DEPTH)
