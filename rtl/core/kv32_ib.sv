    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (can_accept != can_accept_q) begin
                `DEBUG2(`DBG_GRP_FETCH, ("can_accept %0b->%0b (out=%0d discard=%0d req_v=%0b req_r=%0b resp_v=%0b resp_c=%0b)",
                       can_accept_q, can_accept, outstanding, discard_cnt,
                       req_valid, req_ready, resp_valid, resp_consume));
            end

            if (req_valid && req_ready) begin
                `DEBUG2(`DBG_GRP_FETCH, ("issue accepted (out=%0d discard=%0d can_accept=%0b)",
                       outstanding, discard_cnt, can_accept));
            end else if (req_valid && !req_ready) begin
                `DEBUG2(`DBG_GRP_FETCH, ("issue blocked by req_ready=0 (out=%0d discard=%0d can_accept=%0b)",
                       outstanding, discard_cnt, can_accept));
            end else if (!req_valid && can_accept && !flush) begin
                `DEBUG2(`DBG_GRP_FETCH, ("no issue while can_accept=1 (out=%0d discard=%0d)",
                       outstanding, discard_cnt));
            end

            if ((outstanding == CNT_WIDTH'(DEPTH)) && resp_consumed_valid) begin
                `DEBUG2(`DBG_GRP_FETCH, ("full-depth, response consumed (out=%0d discard=%0d can_accept=%0b)",
                       outstanding, discard_cnt, can_accept));
            end
...
`endif
        ...
        if (!rst_n) begin
            ...
            if (flush) begin
                ...
                if (resp_consumed_valid || (resp_consumed && discard_cnt > 0)) begin
                    ...
                    `DEBUG2(`DBG_GRP_FETCH, ("Flush - adding %0d outstanding to discard (adj for consume)", outstanding - 1'b1));
                end else begin
                    if ((discard_cnt + outstanding) > CNT_WIDTH'(DEPTH))
                        discard_cnt <= CNT_WIDTH'(DEPTH);
                    else
                        discard_cnt <= discard_cnt + outstanding;
                    `DEBUG2(`DBG_GRP_FETCH, ("Flush - adding %0d outstanding to discard", outstanding));
               end
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            `DEBUG2(`DBG_GRP_FETCH, ("Push pc=0x%h wrptr=%0d", req_addr, wr_ptr));
        end
    end
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            `DEBUG2(`DBG_GRP_FETCH, ("Discard pc=0x%h rdptr=%0d", pc_fifo[rd_ptr], rd_ptr));
        end else if (resp_consumed && discard_cnt == 0) begin
            ...
            `DEBUG2(`DBG_GRP_FETCH, ("Pop pc=0x%h rdptr=%0d", pc_fifo[rd_ptr], rd_ptr));
        end
    end
